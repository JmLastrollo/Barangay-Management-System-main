...
try {
    ...
    // Kung hindi existing ang ID
    if (!$announcement) {
        $_SESSION['toast'] = ['msg' => 'Announcement not found.', 'type' => 'error'];
        header("Location: resident_announcements.php");
        exit();
    }

    // 3. GET RECENT ANNOUNCEMENTS (Sidebar)
            ...
            <div class="row g-4">
                    ...
                    <div class="announcement-detail-card p-4 h-100">
                        ...
                        <div class="mt-4">
                            <h5 class="fw-bold mb-3 border-bottom pb-2">
                                <i class="bi bi-file-text me-2 text-primary"></i>Details
                            </h5>
                        ...
                        <div class="mt-5 pt-4 border-top">
                            ...
                            <div class="ratio ratio-21x9 shadow-sm rounded overflow-hidden" style="border: 1px solid #eee;">
                                <iframe 
                                    src="https://maps.google.com/maps?q=<?= urlencode($announcement['location']) ?>&t=&z=15&ie=UTF8&iwloc=&output=embed"
                                    allowfullscreen loading="lazy">
                                </iframe>
                            </div>
                            <small class="text-muted mt-2 d-block"><i class="bi bi-info-circle me-1"></i> Map location is based on the address provided.</small>
                        </div>
                        <?php endif; ?>
                ...
                <div class="col-lg-4">
                    <div class="bg-white rounded-4 shadow-sm p-4 sticky-top" style="top: 110px; z-index: 1;">
                        <h5 class="fw-bold mb-4 border-bottom pb-2">Other Recent Updates</h5>
                        
                        <?php if(empty($recents)): ?>
                            <p class="text-muted small">No other announcements found.</p>
                        <?php else: ?>
                            <div class="d-flex flex-column gap-2">
                                <?php foreach($recents as $recent): 
                                    $thumb = !empty($recent['image']) ? "../../uploads/announcements/" . $recent['image'] : "../../assets/img/announcement_placeholder.png";
                                ?>
                                <a href="resident_view_announcement.php?id=<?= $recent['announcement_id'] ?>" class="recent-list-item text-decoration-none">
                                    <img src="<?= $thumb ?>" class="recent-thumb" alt="Thumb">
                                    <div class="recent-details">
                                        <h6 class="text-dark hover-primary"><?= htmlspecialchars($recent['title']) ?></h6>
                        ...
                        <div class="mt-4 pt-3 border-top text-center">
                            <a href="resident_announcements.php" class="btn btn-light btn-sm w-100 fw-bold text-primary">
