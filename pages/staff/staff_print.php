...
// 1. Fetch Data
$sql = "SELECT i.*, r.first_name, r.middle_name, r.last_name, r.address, r.civil_status, r.age, r.gender 
...
// 2. Auto-Update Status if Printing for the first time (optional)
if (in_array($data['status'], ['Pending', 'Payment Verified'])) {
    $upSql = "UPDATE document_issuances 
              SET status = 'Ready for Pickup', approved_at = NOW(), is_printed = 1, processed_by = :sid 
              WHERE issuance_id = :id";
    $upStmt = $conn->prepare($upSql);
    $upStmt->execute([':sid' => $_SESSION['user_id'], ':id' => $id]);
}
...
$controlNo = 'REQ-' . str_pad($data['issuance_id'], 5, '0', STR_PAD_LEFT);
?>
...
<head>
    ...
    <style>
        ...
        .paper {
            background: white; width: 8.5in; height: 11in; margin: 0 auto; padding: 1in;
...
<body>
    ...
    <div class="paper">
        ...
        <div class="content">
            ...
            <p class="indent">
                This is to certify that <strong><?= $fullname ?></strong>, of legal age, 
                <?= strtolower($data['civil_status']) ?>, Filipino, and a resident of 
                <strong><?= $address ?></strong>, is a person of good moral character and standing in this community.
            </p>

            <p class="indent">
                This certification is being issued upon the request of the above-named person for 
                <strong><?= $purpose ?></strong> and for whatever legal intent it may serve.
            </p>

            <p class="indent">
                Issued this <strong><?= $date ?></strong> at Barangay Langkaan II, Dasmariñas City, Cavite.
            </p>
        </div>

        <div class="signature">
            <div class="sign-line">HON. FERNANDO LAUDATO JR.</div><br>
            <small style="margin-right: 40px;">Punong Barangay</small>
        </div>
        
        <div style="position: absolute; bottom: 50px; left: 50px; font-size: 10px; color: gray;">
            Control No: <?= $controlNo ?><br>
            Printed by: Staff ID #<?= $_SESSION['user_id'] ?>
        </div>
    </div>
